...
- [ ] **Sspai tag filtering doesn't work** (found 2026-05-25 trying to add multi-tag scan). `fetchSspaiMatrix` sets `?tag=<中文>` but 4 very different tags (AI / 独立开发 / 创业 / 效率) all return the SAME 10 articles → sspai's matrix endpoint ignores that `tag` param (wrong param name, or the endpoint is a global feed). Impact: the sspai tag picker is cosmetic — every sspai scan returns the same generic matrix feed regardless of pick. NOT shipping multi-tag scan (would 4x cost for 0 extra surface). Fix needs the correct sspai matrix tag/filter param (inspect sspai.com network). Low urgency — founder mostly uses V2EX; the generic feed still has some dev/indie content the AI filter works on.
